...
class FrailtyDataset(Dataset):
    def __init__(self, csv_file, device='cpu'):
        ...
        # 34 Deficit columns based on prepare_frailty_data.py
        self.deficit_cols = [
            ...
            'recuerdo1', 'recuerdo2', 'copiafiguras1', 'orientacion', 'serial7', 'memoria', # 6 Cog
            'bmi_imp', 'ejer_3_por_sem', 'tabaco', # 3 Bio
            'hospitalizacion', 'visita_medica' # 2 Health
        ]
        
        # Preprocess static covariates (normalization and basic imputation)
        # Using median to fill the very few missing values in static demographic variables
...
class BetaVAE(nn.Module):
    def __init__(self, input_dim=34, static_dim=3, hidden_dims=[24, 16], latent_dim=8):
...
if __name__ == "__main__":
    ...
    try:
        data_path = 'frailty_index_data.csv'
        if not os.path.exists(data_path):
            data_path = 'latent_velocity/frailty_index_data.csv'
        ...
        # Using a slightly smaller LR to stabilize training during the annealing transition
        trained_vae = train_model(vae, dataloader, dataset, epochs=80, learning_rate=5e-4, target_beta=0.1, device=device)
        
        # Save the model
        model_path = 'beta_vae_model.pth'
        # If we are in the parent directory, save to the subfolder if it exists
        if os.path.exists('latent_velocity') and os.path.isdir('latent_velocity'):
            model_path = 'latent_velocity/beta_vae_model.pth'
            
        torch.save(trained_vae.state_dict(), model_path)
        print(f"Trained model saved to {model_path}")
        
    except FileNotFoundError:
        print("frailty_index_data.csv not found.")
