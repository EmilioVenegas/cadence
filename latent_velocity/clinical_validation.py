...
def calculate_velocity_magnitude(gpu_traj_path='latent_velocity/latent_velocity_trajectory.csv'):
    print("Loading high-resolution Latent Velocity dataset...")
    df_traj = pd.read_csv(gpu_traj_path)
...
def execute_lmm_clinical_validation(df_traj, empirical_fi_path='latent_velocity/frailty_index_data.csv'):
...
def generate_visualizations(df_traj_full, surv_grouped, cph, hr, velocity_cols):
    ...
    plt.tight_layout()
    plt.savefig("latent_velocity/km_survival_curves.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    df_obs, _ = extract_latent_vectors('latent_velocity/beta_vae_model.pth', 'latent_velocity/frailty_index_data.csv', device)
    ...
    plt.savefig("latent_velocity/archetypal_trajectories.png", dpi=300)
    plt.close()
    print(" -> Saved archetypal_trajectories.png")
    
    # 3. Velocity Heatmaps
    print("Generating Velocity-Domain Heatmaps...")
    df_fi = pd.read_csv('latent_velocity/frailty_index_data.csv')
    df_fi = df_fi.sort_values(by=['cunicah', 'np', 'a_o_ent'])
    ...
    plt.tight_layout()
    plt.savefig("latent_velocity/velocity_domain_heatmap.png", dpi=300)
    plt.close()
    print(" -> Saved velocity_domain_heatmap.png")

def execute_survival_analysis(df_traj_full, master_data_path='simpleMHAS/simpleMHAS.sav', velocity_cols=None):
    ...
    choices = ['Slow_Ager', 'Fast_Ager']
    patient_vmag['Phenotype'] = np.select(conditions, choices, default='Normal')
