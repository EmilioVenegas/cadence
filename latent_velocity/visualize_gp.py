...
def plot_patient_gp(patient_data, gp_results, z_cols, patient_id):
    ...
    out_path = f'plots/patient_{int(patient_id[0])}_{int(patient_id[1])}_gp.png'
...
def main():
    ...
    # Adjust paths if they are relative to the project root instead of latent_velocity folder
    if not os.path.exists(model_path) and os.path.exists(os.path.join('latent_velocity', model_path)):
        model_path = os.path.join('latent_velocity', model_path)
        data_path = os.path.join('latent_velocity', data_path)
