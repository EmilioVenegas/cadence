...
def extract_latent_vectors(model_path, data_path, device='cpu'):
    ...
    df_meta = dataset.data[['cunicah', 'np', 'a_o_ent']].copy()
...
def fit_predict_gp(patient_data, z_cols, t_grid_step=0.1):
    ...
    # We return NaN arrays or skip them.
    if len(t_obs) < 2:
        return None
    ...
    for k, col in enumerate(z_cols):
        ...
        # Accessing nested kernel params: gp.kernel_ is ConstantKernel * RBF + WhiteKernel
        # k1 = ConstantKernel * RBF
        # k1.k2 = RBF
        opt_len_scale = gp.kernel_.k1.k2.length_scale
        
        # We need the weights: alpha_ = (K + sigma_n I)^-1 * y
        # sklearn nicely provides this exact vector as `gp.alpha_`
        alpha = gp.alpha_
        
        # Compute exact cross-covariance derivative matrix: K'(t*, t_obs)
        # Size will be (N_dense, N_obs)
        # K(t*, t_obs) = k(X_dense, X_obs)
        K_cross = gp.kernel_.k1(t_dense, t_obs) # We only differentiate the RBF part, WhiteKernel derivative is 0 off-diagonal
...
if __name__ == "__main__":
    ...
    output_path = 'latent_velocity_trajectory.csv'
    if os.path.exists('latent_velocity') and os.path.isdir('latent_velocity'):
        output_path = 'latent_velocity/latent_velocity_trajectory.csv'
