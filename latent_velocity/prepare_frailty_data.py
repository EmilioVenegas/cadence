...
def map_biometrics(df):
    ...
    if 'tabaco' in df.columns:
        ...
        # If tabaco is missing, we could fallback to fumado, but for now stick to strictly tabaco
...
def prepare_data(filepath, output_path):
    ...
    for col in continuous_cog_cols:
        if col in df_raw.columns:
            # For continuous/larger-scale cognitive tests, only use triple-digit/high-range codes
    ...
    # To prevent longitudinal leakage, impute per wave.
    # We include Age, Sex, and Education as covariates for imputation but extract only items after.
    ...
    if df_imputed[items].isna().sum().sum() > 0:
        ...
        # Again, include covariates for better cross-wave guessing
    ...
    # Preserve the death wave marker while NaNing out state deficits
...
if __name__ == "__main__":
    input_file = '/home/emiliovenegas/Documents/mendel/simpleMHAS/simpleMHAS.sav'
    output_file = 'frailty_index_data.csv'
